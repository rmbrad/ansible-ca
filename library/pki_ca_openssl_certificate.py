...
from ansible.module_utils.six import iterbytes
...
#TODO: move to crypto utils??? with random_serial_number function
...
def random_serial_number(size=16):
    return int_from_bytes(os.urandom(size), 'big') >> 1
...
class SelfSignedCertificate(Certificate):
    ...
    def __init__(self, module):
        ...
        self.notBefore = self.get_relative_time_option(module.params['selfsigned_not_before'], 'selfsigned_not_before')
        self.notAfter = self.get_relative_time_option(module.params['selfsigned_not_after'], 'selfsigned_not_after')
        self.subject_key_identifier = module.params['selfsigned_subject_key_identifier']
        self.authority_key_identifier = module.params['selfsigned_authority_key_identifier']
        self.digest = module.params['selfsigned_digest']
        self.version = module.params['selfsigned_version']
        self.serial_number = random_serial_number()
    ...
    def generate(self, module):
        ...
        if not self.check(module, perms_required=False) or self.force:
            ...
            if self.subject_key_identifier:
                cert.add_extensions([crypto.X509Extension(b'subjectKeyIdentifier', False, to_bytes(self.subject_key_identifier), subject=cert)])
            if self.authority_key_identifier:
                cert.add_extensions([crypto.X509Extension(b"authorityKeyIdentifier", False, to_bytes(self.authority_key_identifier), issuer=cert)])
...
class OwnCACertificateCryptography(Certificate):
    ...
    def __init__(self, module):
        ...
        self.ca_copy_extensions = module.params['ownca_copy_extensions']
        self.ca_basic_constraints = module.params['ownca_basic_constraints']
        self.ca_basic_constraints_critical = module.params['ownca_basic_constraints_critical']
        self.ca_key_usage = module.params['ownca_key_usage']
        self.ca_key_usage_critical = module.params['ownca_key_usage_critical']
        self.ca_extended_key_usage = module.params['ownca_extended_key_usage']
        self.ca_extended_key_usage_critical = module.params['ownca_extended_key_usage_critical']
        self.csr = crypto_utils.load_certificate_request(self.csr_path, backend=self.backend)
    ...
    def generate(self, module):
        ...
        if not self.check(module, perms_required=False) or self.force:
            ...
            if self.ca_copy_extensions == 'copy':
                extensions = { ext.oid: ext for ext in self.csr.extensions }
            else: 
                extensions = {}
            if self.ca_basic_constraints is not None:
                ca, path_length = crypto_utils.cryptography_get_basic_constraints(self.ca_basic_constraints)
                basic_constraints = x509.BasicConstraints(ca, path_length)
                extensions[basic_constraints.oid] = x509.Extension(basic_constraints.oid, self.ca_basic_constraints_critical, basic_constraints)
            if self.ca_key_usage is not None:
                params = crypto_utils.cryptography_parse_key_usage_params(self.ca_key_usage)
                key_usage = x509.KeyUsage(**params)
                extensions[key_usage.oid] = x509.Extension(key_usage.oid, self.ca_key_usage_critical, key_usage)
            if self.ca_extended_key_usage is not None:
                usages = [crypto_utils.cryptography_name_to_oid(usage) for usage in self.ca_extended_key_usage]
                extended_key_usage = x509.ExtendedKeyUsage(usages)
                extensions[extended_key_usage.oid] = x509.Extension(extended_key_usage.oid, self.ca_extended_key_usage_critical, extended_key_usage)
            if self.ca_copy_extensions == 'copyall':
                for extension in self.csr.extensions:
...
class OwnCACertificate(Certificate):
    ...
    def generate(self, module):
        ...
        if not self.check(module, perms_required=False) or self.force:
            ...
            if self.ca_copy_extensions =='copy':
                extensions = { ext.get_short_name(): ext for ext in self.csr.get_extensions()}
            else:
                extensions = {}
            if self.ca_basic_constraints is not None:
                usages = ', '.join(self.ca_basic_constraints)
                extensions[b'basicConstraints'] = crypto.X509Extension(b'basicConstraints', self.ca_basic_constraints_critical, to_bytes(usages))
            if self.ca_key_usage is not None:
                usages = ', '.join(self.ca_key_usage)
                extensions[b'keyUsage'] = crypto.X509Extension(b'keyUsage', self.ca_key_usage_critical, to_bytes(usages))
            if self.ca_extended_key_usage is not None:
                usages = ', '.join(self.ca_extended_key_usage)
                extensions[b'extendedKeyUsage'] = crypto.X509Extension(b'extendedKeyUsage', self.ca_extended_key_usage_critical, to_bytes(usages))
            if self.ca_copy_extensions == 'copyall':
                for ext in self.csr.get_extensions():
                    extensions[ext.get_short_name()] = ext

            #TODO: hack to add authority key identifier
            extensions[b'authorityKeyIdentifier'] = crypto.X509Extension(b"authorityKeyIdentifier", False, b'keyid:always', issuer=self.ca_cert)

            cert.add_extensions(extensions.values())
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            # TODO: added by us
            selfsigned_subject_key_identifier=dict(type='str'),
            selfsigned_authority_key_identifier=dict(type='str'),
            ...
            # TODO: added by us
            ownca_copy_extensions=dict(type='str', choices=['copyall', 'copy', 'none'], default='copyall'),
            ownca_basic_constraints=dict(type='list', elements='str'),
            ownca_basic_constraints_critical=dict(type='bool', default=False),
            ownca_key_usage=dict(type='list', elements='str'),
            ownca_key_usage_critical=dict(type='bool', default=False),
            ownca_extended_key_usage=dict(type='list', elements='str'),
            ownca_extended_key_usage_critical=dict(type='bool', default=False),
